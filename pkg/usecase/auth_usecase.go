...
type Auth interface {
	...
	Signup(c echo.Context, fv *FormValue) (string, error)
...
// generateAndSetToken は、JWT トークンを生成し、それをクッキーに格納する関数です。
func (a *AuthUseCase) generateAndSetToken(c echo.Context, userID string) error {
	// 独自クレーム作成
	...
	if err != nil {
		return xerrors.Errorf("failed to create token with claims: %w", err)
	}
	...
	cookie.Name = "token"
	cookie.Value = t
	cookie.Expires = nowJST.Add(3 * time.Hour)
	cookie.Path = "/"
...
func (a *AuthUseCase) Signup(c echo.Context, fv *FormValue) (string, error) {
	encEmail, err := a.userRepo.Encrypt(fv.Email)
	encPassword, err := a.userRepo.Encrypt(fv.Password)
	user, err := a.userRepo.CreateUser(encEmail, encPassword)
	...
	if err := a.generateAndSetToken(c, user.ID); err != nil {
		return "", fmt.Errorf("token生成失敗", err)
	}
